...
const createArticle = async () => {
    ...
    const questions = [
        ...
        { name: 'content', type: 'text', message: 'Content' },
        {
            ...
            type: 'select',
            message: 'Author',
            choices: authors.map((a) => ({ title: a.display_name, value: a.id }))
        },
        {
            name: 'published',
            type: 'toggle',
    ...
    const info = db
        .prepare(`INSERT INTO articles (title, slug, summary, content, published, created_at, updated_at, author_id)
    ...
    const categoryIds = answers.category_ids || [];
...
const updateArticle = async () => {
    ...
    db.prepare(`UPDATE articles
     ...
     WHERE id = ?`).run(answers.title, answers.summary ?? null, answers.content, answers.published ? 1 : 0, articleId);
    const categoryIds = answers.category_ids || [];
